...
object Application extends Controller {
  ...
  val configForm = Form(
    mapping(
      ...
      { //(_, fs, _, loc, _, pic, _, dev, _, ntw ) => PropertyCheckList(fs,  loc,  pic, dev, ntw)  }
        ( fs,  loc,  pic, dev,  ntw, display, thread, ipc, contact, sensor, account, media, sms) => 
          PropertyCheckList(fs,  loc,  pic, dev,  ntw, display,  thread, ipc, contact, sensor, account, media, sms)   
      }
    ...
    // so we have to define custom binding/unbinding functions
    {
      ...
      => { 
         Configs(k, gc, doStateCutOff, stateCutoff, doTimeCutoff, timeCutoff,//verbose, 
  ...
  def uploadFile = Action(parse.multipartFormData) { implicit request =>
    request.body.file("apkfile").map { picture =>
      ...
      // random
        File.separator  + getFileName(filename) + scala.util.Random.nextInt.toString
     ...
     // (Option[Int],Option[String], Option[String], Int, Option[String], Option[String], String)], path: String  )
      val initialPropertyCheckList = PropertyCheckList(true, true,true,true,true,true,true,true,true, true, true,true,true )//PropertyCheckList("false","false","false","false","false")
  ...
  def config(path: String) = Action { implicit request =>
   ...
   //   {case (k, gc, doStateCutOff, stateCutoff, verbose, doRegex, regex, docheckList, sms) =>  
        ...
        val at = new AnalysisHelperThread(k, gc, doStateCutOff, stateCutoff, doTimeCutoff, timeCutoff,//verbose, 
            path,doRegex, regex, docheckList, properList)//fs,loc,pic,personal,ntw)
            at.start()
        Ok(html.simple(CommonUtils.constrJsonResult(k, gc, doStateCutOff, stateCutoff, doTimeCutoff, timeCutoff,path, doRegex, regex, docheckList, properList)))//fs,loc,pic,personal,ntw)))
      }   
     // }
    )
  }
